                ...
                <div class="card-body">
                    ...
                    @if($product->images->isNotEmpty() || $hasVideo)
                            ...
                            <div class="carousel-inner" role="listbox"
                                ...
                                @if($hasVideo)
                                    <div class="carousel-item active">
                                        <div class="ratio ratio-16x9">
                                            <iframe id="productVideo" src="{{ $videoEmbedUrl }}" class="rounded"
                                ...
                                @foreach($product->images as $index => $image)
                                    <div class="carousel-item {{ !$hasVideo && $index === 0 ? 'active' : '' }}"
                                        ...
                                        <img src="{{ storage_url($image->image_path) }}" alt="{{ $product->name }}"
                        ...
                        <div class="carousel-indicators m-0 mt-2 d-lg-flex d-none position-static h-100">
                            @if($hasVideo)
                                <button type="button" data-bs-target="#carouselExampleFade" data-bs-slide-to="0" aria-label="Video"
                            ...
                            @foreach($product->images as $index => $image)
                                <button type="button" data-bs-target="#carouselExampleFade"
                                    data-bs-slide-to="{{ $hasVideo ? $index + 1 : $index }}" aria-label="Slide {{ $index + 1 }}"
                                    class="w-auto h-auto rounded bg-light {{ !$hasVideo && $index === 0 ? 'active' : '' }}">
                                    <img src="{{ storage_url($image->image_path) }}" class="d-block avatar-xl"
        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="mb-1">
                        <a href="{{ route('inventory.products.show', $product) }}"
                            class="fs-24 text-dark fw-medium">{{ $product->name }}</a>
                    </p>
                ...
                <div class="quantity mt-4">
                    ...
                    <div class="d-flex align-items-center gap-3">
                        <span
                            class="badge bg-{{ $product->quantity > 0 ? 'success' : 'danger' }}-subtle text-{{ $product->quantity > 0 ? 'success' : 'danger' }} py-2 px-3 fs-16">
                ...
                <ul class="d-flex flex-column gap-2 list-unstyled fs-15 my-3">
                    ...
                    @if($product->sku)
                        <li>
                            <i class='bx bx-check text-success'></i> كود المنتج: <span
                                class="text-dark fw-medium">{{ $product->sku }}</span>
                        </li>
                    @endif
                    @if($product->supplier)
                        <li>
                            <i class='bx bx-check text-success'></i> المورد: <span
                                class="text-dark fw-medium">{{ $product->supplier->name }}</span>
                        </li>
                    @endif
                </ul>
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <ul class="d-flex flex-column gap-2 list-unstyled fs-14 text-muted mb-0">
                                @if($product->sku)
                                    <li><span class="fw-medium text-dark">كود المنتج (SKU)</span><span
                                            class="mx-2">:</span>{{ $product->sku }}</li>
                                @endif
                                @if($product->author)
                                ...
                                @if($product->category)
                                    <li><span class="fw-medium text-dark">الفئة الرئيسية</span><span
                                            class="mx-2">:</span>{{ $product->category->name }}</li>
                                @endif
                                @if($product->subcategory)
                                    <li><span class="fw-medium text-dark">الفئة الفرعية</span><span
                                            class="mx-2">:</span>{{ $product->subcategory->name }}</li>
                                @endif
                                @if($product->supplier)
                                    <li><span class="fw-medium text-dark">المورد</span><span
                                            class="mx-2">:</span>{{ $product->supplier->name }}</li>
                                @endif
                                @if($product->purchase_price && auth()->user()->canViewPurchasePrice())
                                    <li><span class="fw-medium text-dark">سعر الشراء</span><span
                                            class="mx-2">:</span>{{ format_currency($product->purchase_price) }}</li>
                                @endif
                                @if($product->retail_price)
                                    <li><span class="fw-medium text-dark">سعر البيع مفرد</span><span
                                            class="mx-2">:</span>{{ format_currency($product->retail_price) }}</li>
                                @endif
                                @if($product->wholesale_price)
                                    <li><span class="fw-medium text-dark">سعر البيع جملة</span><span
                                            class="mx-2">:</span>{{ format_currency($product->wholesale_price) }}</li>
                                @endif
                                <li><span class="fw-medium text-dark">الكمية</span><span
                                        class="mx-2">:</span>{{ $product->quantity }} قطعة</li>
                                @if($product->min_quantity)
                                    <li><span class="fw-medium text-dark">الحد الأدنى</span><span
                                            class="mx-2">:</span>{{ $product->min_quantity }} قطعة</li>
                                @endif
                                ...
                                @endif
                                @if($product->size)
                                    <li><span class="fw-medium text-dark">الحجم</span><span
                                            class="mx-2">:</span>{{ $product->size->label() }}</li>
                                @endif
                                @if($product->page_count)
                                    <li><span class="fw-medium text-dark">عدد الصفحات</span><span
                                            class="mx-2">:</span>{{ $product->page_count }} صفحة</li>
                                @endif
                                @if($product->is_hardcover)
                                    <li><span class="fw-medium text-dark">نوع الغلاف</span><span class="mx-2">:</span>هاردكفر
                                    </li>
                                @endif
                                <li><span class="fw-medium text-dark">الحالة</span><span class="mx-2">:</span>
                                    <span
                                        class="badge bg-{{ $product->is_active ? 'success' : 'danger' }}-subtle text-{{ $product->is_active ? 'success' : 'danger' }}">
