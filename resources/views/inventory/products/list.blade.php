                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center gap-2">
                                                <div
                                                    ...
                                                    @else
                                                        <iconify-icon icon="solar:box-bold-duotone"
                                                            class="fs-20 text-muted"></iconify-icon>
                                                    @endif
                                                </div>
                                                <div>
                                        ...
                                        <td>
                                            <div class="d-flex flex-column gap-1">
                                                @if($product->category)
                                                    <span
                                                        class="badge bg-info-subtle text-info py-1 px-2">{{ $product->category->name }}</span>
                                                @endif
                                                @if($product->subcategory)
                                                    <span
                                                        class="badge bg-secondary-subtle text-secondary py-1 px-2">{{ $product->subcategory->name }}</span>
                                                @endif
                                            ...
                                            <div class="d-flex flex-column gap-1">
                                                @if($product->purchase_price && auth()->user()->canViewPurchasePrice())
                                                    <small class="text-muted">شراء:
                                                        {{ format_currency($product->purchase_price) }}</small>
                                                @endif
                                                @if($product->retail_price)
                                                    <small class="text-dark fw-medium">مفرد:
                                                        {{ format_currency($product->retail_price) }}</small>
                                                @endif
                                                @if($product->wholesale_price)
                                                    <small class="text-muted">جملة:
                                                        {{ format_currency($product->wholesale_price) }}</small>
                                                @endif
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <span
                                                class="badge bg-{{ $product->is_active ? 'success' : 'danger' }}-subtle text-{{ $product->is_active ? 'success' : 'danger' }} py-1 px-2">
                                        ...
                                        <td>
                                            <div class="d-flex gap-2">
                                                <a href="{{ route('inventory.products.show', $product) }}"
                                                    class="btn btn-light btn-sm" title="عرض">
                                                    <iconify-icon icon="solar:eye-broken"
                                                        class="align-middle fs-18"></iconify-icon>
                                                </a>
                                                <a href="{{ route('inventory.products.edit', $product) }}"
                                                    class="btn btn-soft-primary btn-sm" title="تعديل">
                                                    <iconify-icon icon="solar:pen-2-broken"
                                                        class="align-middle fs-18"></iconify-icon>
                                                </a>
                                                <button type="button" class="btn btn-soft-danger btn-sm" title="حذف"
                    ...
                    <div class="row g-0 align-items-center justify-content-between text-center text-sm-start p-3 border-top">
                        <div class="col-sm">
                            <div class="text-muted">
                                عرض <span class="fw-semibold">{{ $products->firstItem() }}</span> إلى <span
                                    class="fw-semibold">{{ $products->lastItem() }}</span> من <span
                                    class="fw-semibold">{{ $products->total() }}</span> نتيجة
                            </div>
                        </div>
